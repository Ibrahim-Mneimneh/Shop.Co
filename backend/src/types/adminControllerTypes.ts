...
export const isValidBase64 = (base64String:string):IIsValidBase64=>{
    try{
      ...
      if(!match)
        return {success:false, base64ErrorMessage:"Invalid base64 format",content:"",type:""
      }
      const base64Content = match[2]
      const base64Type= match[1]
      console.log(base64Type)

      if(!(base64Type==="jpeg" || base64Type==="png"|| base64Type==="jpg")) // Accept only jpeg,png or jpg
        return {success:false, base64ErrorMessage:"Image should be of type jpeg or png",content:"",type:""}
      ...
      if(decodedSize>maxSize){
        return {success:false, base64ErrorMessage:"Image shouldn't exceeds max image size (5MB)",content:"",type:""}
      }
      ...
      if(!decodedDimension.width || !decodedDimension.height){
        return {success:false, base64ErrorMessage:"Invalid image dimensions",content:"",type:""}
      }
      ...
      if (Math.abs(aspectRatio - requiredRatio) > tolerance) {
        return {success:false, base64ErrorMessage:"Image should be having 1:1 ratio",content:"",type:""}
      }
      return {success:true,base64ErrorMessage:"",type:base64Type,content:base64Content}
      }catch(error){
